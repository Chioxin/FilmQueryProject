  ...
  private void startUserInterface(Scanner kb) {
	  ...
	  do {
		  ...
		  if (input < 1 || input > 3) {
			  ...
			  switch (input) {
			case 1: searchFilmByID(kb);
				break;
			case 2: searchByKeyword(kb);
				break;
			default:
				break;
			}
...
//		  System.err.println("Number Format Exception, an integer is required here.");
  ...
  private String getStringFromUser(Scanner kb) {
	  ...
	  input = input.substring(0, input.indexOf(' ')); // we are trying to get the first word from the user, even if they type in 10 words.
  ...
  private void displayFilm(Film myFilm) {
	  ...
	  printDivider();
	  System.out.println("Title: " + myFilm.getTitle() + "(" + myFilm.getRating() + ") Released in " + myFilm.getReleaseYear().substring(0,4));
